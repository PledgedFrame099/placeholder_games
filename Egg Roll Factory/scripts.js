...
function Load() {
  ...
  const data = dataRaw
    ? JSON.parse(dataRaw)
    : { clicks: 0, cps: 0, cpc: 1, xp: 0, xpNeed: 100, level: 1 };
...
function Save() {
  localStorage.setItem(
    "data",
    JSON.stringify({ clicks, cps, cpc, xp, xpNeed, level }),
  );
}
...
function upgrade(resourceCost, increment, type) {
  ...
  let available = getElem("buy_all").checked
    ? Math.floor(clicks / resourceCost)
    : 1;
